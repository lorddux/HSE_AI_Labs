...
float ACleverAIController::ComputeHouseScore(float Distance, float TimeLeft) const
{
	...
	//if (Distance / GetCharacterMaxSpeed() > TimeLeft + 1)
	return -(Distance + 65 * TimeLeft);
	//else
		//return 0;
}
...
void ACleverAIController::Tick(float DeltaSeconds)
{
	...
	//FVector CurrentControllerLocation[GetControllerCount()];
    ...
    for (int id = 0; id < GetControllerCount(); ++id) {
        if (id != ControllerId) {
            ...
            if (Controller) {
				...
				//CurrentControllerLocation[id] = Controller->GetCharacterLocation();
    ...
    for (int i = 0; i < Orders.Num(); ++i) {
		...
		for (int id = 0; id < GetControllerCount(); ++id) {
			if (id != ControllerId) {
				ACleverAIController* Controller = Cast<ACleverAIController>(GetControllerById(id));
				if (GetDistanceBetween(HouseLocations[HouseNumber], Controller->GetCurrentDestination()) < 170.f) bFlag = false;
			}
		}
